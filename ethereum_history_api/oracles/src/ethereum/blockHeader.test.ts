...
async function getHeaderWithHash(
  hardFork: string,
  fixtureName: string
  ...
  const { result: block } = await readObject<GetBlockFixture>(fileName);
  const header = blockToHeader(block);
  return { header, hash: block.hash };
}

describe('calculateBlockHeaderHash', () => {
  it('pre-London block', async () => {
    const { header, hash } = await getHeaderWithHash('frontier', 'first');
    expect(calculateBlockHeaderHash(header)).toBe(hash);
  });
  it('post-London block', async () => {
    const { header, hash } = await getHeaderWithHash('paris', 'usdc');
    expect(calculateBlockHeaderHash(header)).toBe(hash);
  });
});
...
describe('encodeBlockHeader', () => {
  it('pre-London block', async () => {
    const { header } = await getHeaderWithHash('frontier', 'first');
  ...
  it('post-London block', async () => {
    const { header } = await getHeaderWithHash('paris', 'usdc');
    const expectedHeaderRlp =
